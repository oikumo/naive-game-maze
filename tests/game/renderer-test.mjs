...
test("renderer create", () => {
  ...
  sameArrayElements(renderTexture.texture, renderer.getTexture());
});
...
test("renderer clear", () => {
  ...
  sameArrayElements(texture, renderer.getTexture());

  renderer.clear(red);

  const expected = new Uint32Array([red, red, red, red]);
  sameArrayElements(expected, renderer.getTexture());
});
